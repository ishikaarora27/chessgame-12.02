...
Move::Move(Position src, Position dst)
        : src(src), dst(dst), type(Move::MoveType::Standard), taken(nullptr) {
}

Move::Move(Position src, Position dst, Move::MoveType type)
...
Position
Move::get_move_src() const {
    return this->src;
}

Position Move::get_move_dst() const {
    return this->dst;
}
...
void Move::set_taken_move(Piece *piece) {
    ...
    this->taken = piece->clone(); //do we need to clone?
}
